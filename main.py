...
def drawMaze(maze,row,col):
    for r in range(row):
        for c in range(col):
            ...
            pygame.draw.rect(DISPLAYSURF,cellColor, (c*cellWidth, r*cellHeight,cellWidth,cellHeight), width=cellWidth//4)  
moveMade = False
while True:
    ...
    if not moveMade:
        maze, prow, pcol = mL.undoMoveDirection(maze, row, col, prow, pcol, ourMoves.pop())
        mL.printMaze(maze, row, col)
        drawMaze(maze,row,col)
